...
public class Methods {
    ...
    public void ifElementVisibleClickAndControl(String key, String element, String control){
        if(isElementVisible(getBy(key),1)){
            ...
            logger.info("Elemente tıklanıp kontrol edildi");
        }
    }
    ...
    public void ifVisibleSelectRandomBeden(String key,String key1,String key2){
        if (isElementVisible(getBy(key),1)){
            clickElement(getBy(key));
            logger.info("Tek Beden seçildi");
        }
        else if(isElementVisible(getBy(key1),1)){
            clickElement(getBy(key1));
            logger.info("Tek Kalan seçildi");
        }
        else if (isElementVisible(getBy(key2),1)){
            randomSelect(key2);
            logger.info("Random seçildi");
        }
        else {
            logger.error("Ürün yok hata");
        }}
